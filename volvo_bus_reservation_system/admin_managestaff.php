...
if (isset($_GET['did'])) {
	...
	delete($q);
	$r="delete from login where login_id='$lid'";
	delete($r);
}
...
<?php }else{ ?>
	<table class="table" style="width:500px;color: white">
		...
		<tr>
			<th>Gender</th>
			<td style="color: white"><input type="radio" required=""  class="btn btn-success" name="gen" value="female">Female
			<input type="radio" required="" class="btn btn-success" name="gen" value="male">male
			<input type="radio" required=""  class="btn btn-success" name="gen" value="other">Others</td>
		</tr>
...
<center>
    ...
    foreach ($res as $row) {?>
    	<tr>
            ...
            <td><a class="btn btn-success" href="?did=<?php echo $row['staff_id'] ?>&lid=<?php echo $row['login_id'] ?>">Delete</a></td>
